...
package com.ldt.navigation.action.util;
...
public class SnowflakesEffect {
    ...
    public void onDraw(View parent, Canvas canvas) {
        ...
        if (mRandom.nextFloat() > 0.7f && particles.size() < 100) {
            int statusBarHeight = (Build.VERSION.SDK_INT >= 21 ? AndroidUtilities.statusBarHeight : 0);
            float cx = mRandom.nextFloat() * parent.getMeasuredWidth();
            float cy = statusBarHeight + mRandom.nextFloat() * (parent.getMeasuredHeight() - AndroidUtilities.dp(20) - statusBarHeight);

            int angle = mRandom.nextInt(40) - 20 + 90;
            ...
            newParticle.scale = mRandom.nextFloat() * 1.2f;
            newParticle.type = mRandom.nextInt(2);

            newParticle.lifeTime = 2000 + mRandom.nextInt(100);
            newParticle.velocity = 20.0f + mRandom.nextFloat() * 4.0f;
