...
public class FireworksEffect {
    ...
    public void onDraw(View parent, Canvas canvas) {
        ...
        if (mRandom.nextBoolean() && particles.size() + 8 < 150) {
            ...
            for (int a = 0; a < 8; a++) {
                int angle = mRandom.nextInt(270) - 225;
                ...
                newParticle.scale = Math.max(1.0f,mRandom.nextFloat() * 1.5f);
                newParticle.type = 0;//Utilities.random.nextInt(2);

                newParticle.lifeTime = 1000 + mRandom.nextInt(1000);
